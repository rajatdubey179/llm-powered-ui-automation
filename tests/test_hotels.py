...
def test_hotels_page_loads(page):
    ...
    hotels.open()
    assert "stay" in page.url.lower() or page.title() != ""


def test_search_hotels_returns_results(page):
    hotels = HotelsPage(page)
    hotels.open()
    hotels.search(destination="Dubai", checkin="2025-09-20", checkout="2025-09-25")
...
def test_click_hotel_opens_detail(page):
    ...
    assert "hotel" in page.url.lower() or "stay" in page.url.lower(), "Expected hotel detail page"
